...
dir.create("ctx_dge_bacon")


# BACON analsis CTX
dge <- readxl::read_excel("original_dge/CTX_DGE_FullStats.xlsx")
...
CTX_DGE_Original <- CTX_FullTab_Bacon %>%
                  mutate(Abs = abs(log2FoldChange_original)) %>%
                  filter(padj_original < 0.05 & Abs > 0.3) %>%
                  arrange(desc(Abs))

CTX_DGE_Bacon <- CTX_FullTab_Bacon %>%
                  mutate(Abs = abs(log2FoldChange_corrected)) %>%
                  filter(padj_corrected < 0.05 & Abs > 0.3) %>%
                  arrange(desc(Abs))


openxlsx::write.xlsx(CTX_FullTab_Bacon, 
                     file = "ctx_dge_bacon/CTX_DGE_FullStats_BACON.xlsx", 
                     colNames = TRUE,
                     rowNames = FALSE, 
                     borders = "columns",
                     sheetName="Stats")

openxlsx::write.xlsx(CTX_DGE_Original, 
                     file = "ctx_dge_bacon/CTX_DGE_Original.xlsx", 
                     colNames = TRUE,
                     rowNames = FALSE, 
                     borders = "columns",
                     sheetName="Stats")

openxlsx::write.xlsx(CTX_DGE_Bacon, 
                     file = "ctx_dge_bacon/CTX_DGE_Bacon.xlsx", 
                     colNames = TRUE,
                     rowNames = FALSE, 
                     borders = "columns",
                     sheetName="Stats")
...
pdf("ctx_dge_bacon/Boxplots_TopGenes_CTX_Bacon.pdf",width=6,height=5,useDingbats=FALSE)
...
pdf("ctx_dge_bacon/Vulcano_Plot_CTX_Bacon.pdf",width=6,height=6,useDingbats=FALSE)
ggscatter(df, 
...
# heatmap
mat <- p[rownames(p)%in% CTX_DGE_Bacon$Gene,]
anno <- pd
Genotype        <- c("red", "black")
names(Genotype) <- c("Control", "ADHD")
anno_colors <- list(Genotype = Genotype)
pdf("ctx_dge_bacon/Heatmap_CTX_Bacon.pdf",width=4,height=6)
pheatmap(mat,scale="row",show_rownames = F,annotation=anno,annotation_colors = anno_colors)
dev.off()

# QQ plots
dir.create("ctx_dge_bacon/qqplots_comparative")

pdf("ctx_dge_bacon/qqplots_comparative/CTX_Bacon_Scatter.pdf",width=5,height=5)
ggscatter(CTX_FullTab_Bacon, 
...
pdf("ctx_dge_bacon/qqplots_comparative/CTX_Bacon_QQ_pvals.pdf",width=5,height=3)
plot(bc, type="qq")
dev.off()


pdf("ctx_dge_bacon/qqplots_comparative/CTX_Bacon_QQplots.pdf",width=8,height=5)
par(mfrow = c(1, 2))
...
# Upset Plot
CTX_DGE_Original <- readxl::read_excel("ctx_dge_bacon/CTX_DGE_Original.xlsx")
CTX_DGE_Bacon <- readxl::read_excel("ctx_dge_bacon/CTX_DGE_Bacon.xlsx")

tab1 <- CTX_DGE_Original %>%
...
pdf("ctx_dge_bacon/Upset_Plot_Intersection.pdf", width = 6, height = 4)
