...
mod=read.table("str_dge_bacon/STR_Dge_Bacon_HumanID.txt",sep="\t",header=T)
...
dir.create("str_dge_bacon/GO_STR/")
dir.create("str_dge_bacon/GO_STR/GO_BP/")
dir.create("str_dge_bacon/GO_STR/GO_MF/")
dir.create("str_dge_bacon/GO_STR/GO_CC/")

net=levels(mod$Direction)
for (i in 1:length(dfBP)) write.table(dfBP[[i]], file = paste("str_dge_bacon/GO_STR/GO_BP/BP_", net[[i]], ".txt",sep = ""),sep="\t",quote=F)
for (i in 1:length(dfMF)) write.table(dfMF[[i]], file = paste("str_dge_bacon/GO_STR/GO_MF/MF_", net[[i]], ".txt",sep = ""),sep="\t",quote=F)
for (i in 1:length(dfCC)) write.table(dfCC[[i]], file = paste("str_dge_bacon/GO_STR/GO_CC/CC_", net[[i]], ".txt",sep = ""),sep="\t",quote=F)

# Create Plots
setwd("str_dge_bacon/GO_STR/")
...
pdf("BP_STR_enrichment.pdf",width=8,height=3,useDingbats=FALSE)
...
pdf("MF_STR_enrichment.pdf",width=8,height=3,useDingbats=FALSE)
ggscatter(top_labelled, x = "log", y = "OddsRatio",
   color = "Class", palette = c("royalblue", "royalblue", "royalblue"),size = 2,
   label = "Term", repel = TRUE,font.label = c(8, "plain"))+
xlab("-log10(p-value)")+ 
...
pdf("CC_STR_enrichment.pdf",width=8,height=3,useDingbats=FALSE)
ggscatter(top_labelled, x = "log", y = "OddsRatio",
   color = "Class", palette = c("royalblue", "royalblue", "royalblue"),size = 2,
   label = "Term", repel = TRUE,font.label = c(8, "plain"))+
xlab("-log10(p-value)")+ 
